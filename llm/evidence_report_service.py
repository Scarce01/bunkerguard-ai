...
_SEVERITY_WEIGHT = {"CRITICAL": 1.0, "HIGH": 0.8, "MEDIUM": 0.55, "LOW": 0.35}


def _fallback_confidence(session: dict, anomalies: list[dict]) -> float:
    """Derive a fallback confidence from real signal — no magic constant.

    Two ingredients:
      * Mean per-rule confidence of the detected anomalies, weighted by
        severity (so a single CRITICAL with conf=0.95 dominates a noisy LOW).
      * The risk engine's own score, normalised to [0,1] — high scores mean
        the deterministic engine was sure, which the LLM fallback inherits.

    No anomalies + no score → 0.30 (we're guessing). Both present and
    aligned → up to ~0.92. Clamped to [0.2, 0.92] so we never claim
    certainty we can't justify.
    """
    if anomalies:
        weighted_conf = []
        for a in anomalies:
            sev = (a.get("severity") or "").upper()
            w = _SEVERITY_WEIGHT.get(sev, 0.4)
            c = float(a.get("confidence") or 0.0)
            weighted_conf.append(w * c if c > 0 else w * 0.6)
        anomaly_conf = sum(weighted_conf) / len(weighted_conf)
    else:
        anomaly_conf = None

    score = session.get("risk_score")
    risk_conf = (float(score) / 100.0) if isinstance(score, (int, float)) else None

    parts = [p for p in (anomaly_conf, risk_conf) if p is not None]
    if not parts:
        return 0.30
    raw = sum(parts) / len(parts)
    return max(0.20, min(0.92, round(raw, 2)))


def build_fallback_explanation(session: dict, anomalies: list[dict]) -> dict:
    sorted_a = sorted(anomalies, key=lambda a: _SEVERITY_ORDER.get(a.get("severity"), 4))
    ...
    return {
        ...
        "supplier_note": "",
        "confidence": _fallback_confidence(session, anomalies),
    }
